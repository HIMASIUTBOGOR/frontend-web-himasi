...
var requestSource = async (path) => {
  const source = await readFile2(path, "utf-8");
...
var createMagicString = (code) => {
  return typeof code === "string" ? new MagicString2(code) : code;
};
...
var configTypes = (options = {}) => {
  return {
    ...
    async buildStart() {
      ...
      configPath = configPath ? resolve2(configPath) : configPath;
      const module = typeModule(configPath);
      await mkdir("node_modules/.vuestic", { recursive: true });
      await Promise.all([
        writeFile2("node_modules/.vuestic/index.d.ts", indexTs(), {
          encoding: "utf-8",
          flag: "w"
        }),
        writeFile2("node_modules/.vuestic/config.d.ts", module, {
          encoding: "utf-8",
          flag: "w"
        })
      ]);
...
import { MagicString as MagicString4 } from "@vue/compiler-sfc";
  ...
  return [
    {
      ...
      transform(code, id) {
        ...
        const ms = new MagicString4(code);
