...
const columns = defineVaDataTableColumns([
  ...
  { label: "departemen", key: "departemen", sortable: true },
  { label: "title", key: "title", sortable: true },
  { label: "desc", key: "desc", sortable: true },
  { label: "is_active", key: "is_active", sortable: true },
  { label: "photo", key: "photo", sortable: true },
  { label: " ", key: "actions", align: "right" },
]);

const props = defineProps({
  proker: {
    type: Array as PropType<Proker[]>,
    required: true,
  },
...
const emit = defineEmits<{
  (event: "edit-proker", proker: Proker): void;
  (event: "delete-proker", proker: Proker): void;
  (event: "update:sortBy", sortBy: Sorting["sortBy"]): void;
  (event: "update:sortingOrder", sortingOrder: Sorting["sortingOrder"]): void;
}>();

const proker = toRef(props, "proker");
...
const onDeleteProker = async (proker: Proker) => {
  const agreed = await confirm({
    title: "Delete proker",
    message: `Are you sure you want to delete ${proker.title}?`,
    okText: "Delete",
    cancelText: "Cancel",
    size: "small",
    maxWidth: "380px",
  ...
  if (agreed) {
    emit("delete-proker", proker as Proker);
  }
};
</script>
...
<template>
  ...
  >
    ...
    <template #cell(departemen)="{ rowData }">
    ...
    <template #cell(photo)="{ rowData }">
      <div class="flex items-center gap-2 max-w-[230px] ellipsis">
        <img :src="rowData.photo" :alt="rowData.id" />
      </div>
    </template>
    ...
    <template #cell(actions)="{ rowData }">
      <div class="flex gap-2 justify-end">
        <VaButton
          ...
          size="small"
          icon="mso-edit"
          aria-label="Edit proker"
          @click="$emit('edit-proker', rowData as Proker)"
        />
        <VaButton
          v-if="canDelete"
          preset="primary"
          ...
          icon="mso-delete"
          color="danger"
          aria-label="Delete proker"
          @click="onDeleteProker(rowData as Proker)"
        />
      </div>
    </template>
  </VaDataTable>
